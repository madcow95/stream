...
$(document).ready(function() {
	var operForm = $("#operForm"); 
		$("button[data-oper='modify']").on("click", function(e){
	  	operForm.attr("action","${ctx}/board/adminmodify").submit();
	});
...
$(document).ready(function() {
	...
	function toList() {
		...
		location.href="${ctx}/board/list?articleno="
	}
</script>
</head>
<body>
      ...
      <div class="panel-body">
        <!-- <form role="form" action="/board/register" method="post"> -->
          <div class="form-group">
            ...
            <input type="hidden" name='artno' value="${aboardList.articleno }">
          </div>
          <div class="form-group">
            <label>Subject</label> <input class="form-control" name='title' value="${aboardList.title }" disabled="disabled">
          </div>

          <div class="form-group">
            <label>Content</label>
            <textarea class="form-control" rows="3" name='content' disabled="disabled">${aboardList.content }</textarea>
          </div>
          ...
          <div class="form-group">
            <label>RegDate</label> <input class="form-control" name='regdate' value="${aboardList.regdate }" disabled="disabled">
          </div>
          <div class="form-group" style="margin: 0 auto; text-align: center;">
          <sec:authentication property="principal" var="sec"/>
          	<sec:authorize access="hasRole('ROLE_ADMIN')">
          		<%-- <c:if test="${sec.username eq aboardList.id}"> --%>
		          <button class="btn btn-primary" data-oper="modify" id="modify">Modify</button>
          		<%-- </c:if> --%>
          	</sec:authorize>
	          <button class="btn btn-default" style="border: 1px solid #000000;" data-oper="list">List</button>
          </div>
          <form action="${ctx}/board/adminmodify" method="get" id="operForm">
          	<input type="hidden" id="articleno" name="articleno" value="${aboardList.articleno}">
          	<input type="hidden" id="pageNum" name="pageNum" value="${acri.pageNum}">
          	<input type="hidden" id="type" name="type" value="${acri.type}">
          	<input type="hidden" id="keyword" name="keyword" value="${acri.keyword}">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-body">
            	<div class="form-group">
            		<label>댓글내용</label>
            		<input class="form-control" name="replytext" placeholder="댓글을 작성해주세요">
            	</div>
            	<div class="form-group">
            		<label>작성자</label>
